...
def kernelRollingAverage(datapointsT, kernel=[[-604800,0,604800], [0,1,0]], certainties=[]): #datapointsT should already be filtered for None's
    if len(certainties) == 0:
        certainties = [1 for i in range(len(datapointsT[0]))]
    d = datapointsT
    out = []
    ...
    for p in range(len(d[0])):
        ...
        for otherP in [x for _,x in sorted(zip(np.abs(timeDisps),[p for p in range(len(d[0]))]))][:pointsPerTerm-1]:
            ...
            window[1].append(np.interp([timeDisps[otherP]], np.array(kernel[0]) * winMult, kernel[1])[0] * certainties[otherP]) #append the weight to the window
...
def drawMonths():
    ...
    for pair in range(len(mTs) - 1):
        ...
        ax1.add_patch(rect)
        ax1.text((mTs[pair] + mTs[pair+1]) / 2, 0, dates[pair].strftime('%B'), fontsize=12, horizontalalignment='center')
    pairPerc = 1
